...
import { ChevronDown, Briefcase, MapPin, ArrowRight, CheckCircle2, Clock, LayoutDashboard, UserCircle, Settings } from 'lucide-react';
import type { ApplicationStage } from '../../types/database';
import { DashboardLayout, SideItem, StatCard } from '../../components/DashboardShared';
...
export default function CandidateDashboard() {
  ...
  const profileCompletion = (() => {
    ...
    const fields = [
      ...
      profile.cgpa,
      profile.hometown,
      applications.length > 0, // has applied to at least 1 job
    ];
  ...
  return (
    <DashboardLayout
      ...
      headerTitle={`Welcome back, ${profile?.full_name?.split(' ')[0]}`}
      headerSub="Here's your application overview."
      headerActions={<button onClick={() => navigate('/candidate/profile')} className="btn btn-primary" style={{ fontSize: '0.85rem' }}>Edit Profile</button>}
      roleLabel="Candidate"
    >
      {/* Stats */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', animation: 'slideInRight 0.5s var(--ease-apple) 0.2s both' }}>
          ...
          <div style={{ background: '#000', borderRadius: 'var(--radius-xl)', padding: '1.25rem', color: 'white', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ height: '6px', background: 'rgba(255,255,255,0.08)', borderRadius: '3px', overflow: 'hidden', marginBottom: '0.5rem' }}>
              <div style={{ width: `${profileCompletion}%`, height: '100%', background: 'var(--primary)', borderRadius: '3px', transition: 'width 1s var(--ease-apple)' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem' }}>
              <span style={{ color: 'rgba(255,255,255,0.5)' }}>{profileCompletion}%</span>
              <Link to="/candidate/profile" style={{ color: 'var(--primary)', fontWeight: 600 }}>{profileCompletion >= 100 ? '✓ Done' : 'Complete'}</Link>
            </div>
          </div>
        </div>
      </div>
